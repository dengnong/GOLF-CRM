...
<body>
    ...
    <div class="alert alert-info">当前位置<b class="tip"></b>商务中心<b class="tip"></b>扣除权益</div>
  
  <form name="subEquity" action="<%=path %>/businesscenter/UntreatedorderList" method="post">
  <input name="operation" type="hidden" id="operation" value="subEquity"/>
  <% 
  int ord_number = Integer.parseInt(String.valueOf(request.getAttribute("ord_Number")));
	 System.out.println("ord_Number:"+"    "+ord_number+ "come in subEquity.jsp");
		 %>
 ...
 %>
  ...
  <tbody>
    ...
    <tr>
      ...
      <input name="cus_Phone" type="hidden" id="cus_Phone" value=<%=tborder.getVip_Phone()%> >
    </tr>
    <tr>
      <td align="right">会员卡号：</td>
      <td><%=tborder.getPro_Number() %></td>
      <input name="pro_Number" type="hidden" id="pro_Number" value=<%=tborder.getPro_Number()%> >
    </tr>
    <tr>
    ...
    <tr>
     ...
     <tr>
      ...
      <td><%=tborder.getOrd_Shouldequity() %></td>
      <input name="ord_shouldEquity" type="hidden" id="ord_shouldEquity" value=<%=tborder.getOrd_Shouldequity()%> >
    </tr>
    <tr>
...
<%} 
    ...
    <input type="submit" name="submit" value="扣除权益" />
     <p>&nbsp;</p>
     <p>&nbsp;</p>
    <td>&nbsp;</td>
    </p>
